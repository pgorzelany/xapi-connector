...
(function() {
  ...
  Wrapper = (function(_super) {
    ...
    function Wrapper(server_url, conn_port, stream_port, username, password) {
      ...
      this._stream_emitter = new Emitter();
      return;
    }
    ...
    Wrapper.prototype.onOpen = function() {
      ...
      this.monitorConn();
      this.on('ping', (function(_this) {
        return function(req, res) {
          return _this.env.last_ping = new Date().getTime();
        };
      })(this));
    ...
    Wrapper.prototype.onMessage = function(msg) {
      msg = JSON.parse(msg);
      if (msg.status === true) {
        this._emitter.emit([this.env.messages[msg.customTag].command], this.env.messages[msg.customTag], msg);
      } else {
        this.onApiError(msg);
      }
    };
    ...
    Wrapper.prototype.monitorConn = function(timeout) {
      var now;
      now = new Date().getTime();
      if (timeout == null) {
        timeout = 0;
      }
      if (now - this.env.last_ping <= timeout || this.env.last_ping === void 0) {
        this.ping();
        this.last_ping = new Date().getTime();
        timeout = this.conn.dispatcher.que.length * this.conn.dispatcher.delay + 3000;
        setTimeout((function(_this) {
          return function() {
            return _this.monitorConn(timeout);
          };
        })(this), timeout);
      } else {
        this.disconnect();
    ...
    Wrapper.prototype.onStream = function(event, callback) {
      this._stream_emitter.on(event, callback);
    };
    ...
    Wrapper.prototype.getCandles = function(args) {
      this.conn.send(this.buildCommand('getCandles', args));
    };
